...
while 1
    ...
    for p = 1:rows
        ...
        if any(isnan(x(:))) %check x for NaN values
            ...
            vertDif(p) = -1;  % editIvan_20200422; if all values are nan, then this empty row must not be selected
        else
    ...
    for q = 1:cols
        ...
        if any(isnan(x(:)))
            %horizDif(q) = 0;
            horizDif(q) = -1;  % editIvan_20200422; if all values are nan, then this empty column must not be selected
        else
    ...
    if printMatrixAtoExcel == 1
        rowIndex = rowIndex + length(A) + 1  % editIvan_20200422
        xlswrite(filename,{'maxVert ',num2str(maxVert),'vertIndex ',num2str(vertIndex)},'sheet1',strcat('A',num2str(rowIndex)));  % editIvan_20200422
        rowIndex = rowIndex + 1  % editIvan_20200422
        xlswrite(filename,{'maxHoriz ',num2str(maxHoriz),'horizIndex ',num2str(horizIndex)},'sheet1',strcat('A',num2str(rowIndex)));  % editIvan_20200422
    end
