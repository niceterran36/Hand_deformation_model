...
%% register library - Dell Labtop
clc
clear all
addpath(genpath('../external'));
addpath('C:\Users\EDT-jhy\Documents\GitHub\Hand_deformation_model\data');
addpath('C:\Users\EDT-jhy\Documents\GitHub\Hand_deformation_model\data_SW');
addpath('C:\Users\EDT-jhy\Documents\GitHub\Hand_deformation_model\functions');
addpath('C:\Users\EDT-jhy\Documents\GitHub\Hand_deformation_model\external\registration');
format shortG
...
LMp = function_get_LM_from_iges('LM_for_palm_cor_est.igs');
Template_LMp = zeros(10,1);
m = size(mesh.vertices, 1);

for i=1:10
delta = mesh.vertices - repmat(LMp(i, :), m, 1);
distances = sum(delta .^ 2, 2);
[~, j] = min(distances);
Template_LMp(i,:) = j;
end
for i = 1:10
LMp(i,:) = mesh.vertices(Template_LMp(i),:);
end

LM_before_transform = LMp;
...
%% Palm ICP registration

% LMp
% for i = 1:10
% LMp(i,:) = hand_template.vertices(Template_LMp(i),:);
% end
% LMp %landmakr before transformation
% 
% % palm segment based LMp index detection
% palm_vertices = vertices;
% palm_faces = faces;
% mm = size(palm_vertices,1);
% 
% for i=1:10
% delta = palm_vertices - repmat(LMp(i, :), mm, 1);
% distances = sum(delta .^ 2, 2);
% [~, j] = min(distances);
% palm_LMp_idx(i,:) = j;
% end
...
% 
% % apply transformation matrix
% [regParams,Bfit,ErrorStats] = absor(LM_bf_register,LM_af_register);
% % MCP ID
% MCP_ID = [4 8 12 16];
% 
% for i = 1:4
% hand_template.spheres{1,MCP_ID(i)}.center = apply_matrix(regParams.M, hand_template.spheres{1,MCP_ID(i)}.center) % update current centers
% end 
% 
% for i = 1:4
% centers_MCP(i,:) = hand_template.spheres{1,MCP_ID(i)}.center;
% end
